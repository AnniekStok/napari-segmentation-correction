from napari_segmentation_correction.erosion_dilation_widget import erode_labels, expand_labels_skimage
from napari_segmentation_correction.connected_components import keep_largest_cluster, keep_largest_fragment_per_label, connected_component_labeling
from napari_segmentation_correction.image_calculator import add_images, subtract_images, multiply_images, divide_images, logical_and, logical_or
...
from napari_segmentation_correction.process_actions_helpers import process_action, process_action_seg
...
def test_threshold(img_2d, img_3d, img_4d, tmp_path):
    ...
    with patch('napari_segmentation_correction.process_actions_helpers.QFileDialog.getExistingDirectory') as mock_dialog:
...
def test_erode_dilate(action, img_2d, img_3d, img_4d, tmp_path):
    ...
    expected = action(img[0].compute(), diam=3, 
        iterations=1)
    np.testing.assert_array_equal(result[0], expected)

@pytest.mark.parametrize("action", [compute_boundaries, keep_largest_cluster, keep_largest_fragment_per_label, connected_component_labeling])
...
@pytest.mark.parametrize("action", [add_images, subtract_images, multiply_images, divide_images, logical_and, logical_or])
